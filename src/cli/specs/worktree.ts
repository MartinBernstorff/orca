...
export const WORKTREE_COMMAND_SPECS: CommandSpec[] = [
  ...
  },
  {
    ...
    usage:
      'orca worktree create --name <name> [--repo <selector>|--project <id> [--host <host-id>]|--project-host-setup <id>] [--agent <id>] [--prompt <text>] [--setup run|skip|inherit] [--base-branch <ref>] [--issue <number>] [--linear-issue <identifier-or-url>] [--comment <text>] [--workspace-status <column>] [--parent-worktree <selector>] [--no-parent] [--run-hooks] [--activate] [--json]',
    allowedFlags: [
      ...GLOBAL_FLAGS,
      'repo',
      'project',
    ...
    notes: [
      ...
      'Pass --workspace-status to open the workspace in a board column other than the default; it takes the column name, e.g. --workspace-status "Human review", or its configured id. An unknown value is rejected with the available columns listed.',
